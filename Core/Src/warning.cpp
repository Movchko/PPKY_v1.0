...
static int FindItem(uint8_t zone, uint8_t h_adr, uint8_t v_l_adr, uint8_t mcu_d_type, uint8_t v_d_type)
{
	for (uint8_t i = 0u; i < WARN_MAX_ITEMS; i++) {
		if (!g_items[i].used) {
			continue;
		}
		if (g_items[i].zone == zone &&
...
static void UpsertItem(uint8_t zone, uint8_t h_adr, uint8_t v_l_adr, uint8_t mcu_d_type, uint8_t v_d_type, uint8_t line_state, uint32_t now_ms)
{
	int idx = FindItem(zone, h_adr, v_l_adr, mcu_d_type, v_d_type);
	if (idx >= 0) {
...
static void MarkRecovered(uint8_t zone, uint8_t h_adr, uint8_t v_l_adr, uint8_t mcu_d_type, uint8_t v_d_type, uint32_t now_ms)
{
	int idx = FindItem(zone, h_adr, v_l_adr, mcu_d_type, v_d_type);
	if (idx >= 0) {
...
static uint8_t IsItemStillFaulty(const WarningItem& it)
{
	for (uint8_t mi = 0u; mi < g_active_devices_count; mi++) {
		...
		for (uint8_t vi = 0u; vi < m->vdev_count; vi++) {
			...
			if (v->v_l_adr == it.v_l_adr && v->v_d_type == it.v_d_type) {
	...
	for (uint8_t mi = 0u; mi < g_active_devices_count; mi++) {
		...
		for (uint8_t vi = 0u; vi < m->vdev_count; vi++) {
			...
			if (IsFaultLineState(v->line_state)) {
				UpsertItem(m->dev.zone, m->dev.h_adr, v->v_l_adr, m->dev.d_type, v->v_d_type, v->line_state, now_ms);
			} else {
				MarkRecovered(m->dev.zone, m->dev.h_adr, v->v_l_adr, m->dev.d_type, v->v_d_type, now_ms);
			}
			v->status_changed = 0u;
		}
	}
...
static uint8_t BuildUiPayload(char (*big_titles)[16], char (*details)[ZONE_NAME_SIZE + 1])
{
	...
	for (uint8_t a = 1u; a < on; a++) {
		...
		while (b > 0u) {
			...
			else if (l.zone == r.zone && l.h_adr == r.h_adr && l.v_l_adr == r.v_l_adr && l.v_d_type > r.v_d_type) greater = 1u;
			if (!greater) {
				break;
			}
			order[b] = order[b - 1u];
	...
	for (uint8_t i = 0u; i < on && count < WARN_MAX_ITEMS; i++) {
		...
		if (it.zone < ZONE_NUMBER) {
			strncpy(zone_name, reinterpret_cast<const char*>(PPKYConfig.zone_name[it.zone - 1]), ZONE_NAME_SIZE);
